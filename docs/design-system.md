...
- Work: a centered heading, shared category/search controls, six featured folders and six archive folders. Curated order is preserved until an explicit date sort. Four private-work cards use original illustrations, sans-serif headlines and the article index’s clean art-and-copy layout.
...
Folder transforms and hover lifts use ease-out and spring-like curves. Pointer movement locally enlarges dots within a soft radius. A canvas mask clears the decoration beneath text, paragraphs, headings and UI surfaces, including while content moves. Touch and reduced-motion devices retain only static dots; rendering stops when idle. Without JavaScript, the page uses the plain canvas color. Soft shadows replace decorative borders. Mouse focus avoids oversized rings while keyboard focus remains visible. The game starts only on input. Reduced-motion styles suppress decorative movement.

The Interactions section of Play Lab contains six working demos: shape switching, selectable spring motion, a tilting/turning studio pass, click celebration, an adjustable spinning petal bloom and a tactile ripple pond with three palettes. A compact segmented control links the canvas and experiments. Celebration has a clear burst and halo, while pond clicks create thick expanding ripples. Keyboard, touch and reduced-motion feedback remain available.
...
Capabilities open the first row when visible, fill a progress ring over nine seconds, then cycle forward. Selecting any row closes the previous one and restarts the timer. Offscreen and hidden tabs stop the cycle. Home’s personal cards share one integrated Quick Ask surface; mobile users can swipe through all five cards. The About photo fan lifts, tilts and pins individual photos with pointer, tap and keyboard controls.
