...
Quick Ask helps visitors find Baivab’s public background, work, skills, education, writing and contact details. The input and expanding reply area share one opaque rounded surface, with a visible thinking state and links to relevant pages. Suggestions and repeated questions are available without retyping; repeated answers are cached for the current page visit.
...
Existing account quotas apply. This configuration does not purchase or upgrade a plan. If inference is unavailable, slow, invalid or over quota, the visitor gets an approved portfolio answer. Other static hosts use the same fallback. The local development server exercises the endpoint without remote inference or usage charges.
...
The server retrieves a bounded set of candidate facts. Explicitly unsupported and instruction-changing questions are declined before inference. AI chooses one or two IDs from those candidates using [Workers AI JSON mode](https://developers.cloudflare.com/workers-ai/features/json-mode/), with a schema limited to the candidate IDs. The server validates both structured-object and text responses and returns only the corresponding approved text and local links. Model-written claims, arbitrary URLs and malformed responses never appear in the answer. The API mode records how facts were selected. The interface uses Baivab’s approved first-person voice and does not display an assistant or AI identity. It is automated portfolio content, not a live messaging inbox.
...
Whole-message greetings, thanks and basic introductions resolve without inference. The browser gives every message a two-second minimum sending/thinking state, including cached answers. Replies remain visible for ten seconds, then close smoothly; clicking outside hides them immediately. New submissions cancel stale work and dismissal timers. The copy uses Baivab’s first-person voice. Greetings cannot bypass restrictions when combined with unrelated questions or requests for private information. Thirteen server test groups cover this behavior and the existing grounding, request and fallback controls.
