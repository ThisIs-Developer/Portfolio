...
export function renderPages(data, metadata) {
  ...
  const all = [...projects, ...experiments].map((p) =>
    p.id === "ams"
      ? {
          ...p,
  ...
  const nav = (archive) =>
    `<a class="skip-link" href="#main-content" tabindex="0">Skip to content</a><header class="site-header"><div class="nav-shell"><button class="menu-toggle" type="button" aria-controls="site-nav" aria-expanded="false" hidden><span class="menu-glyph" aria-hidden="true"><i></i><i></i></span><span data-menu-label>Menu</span></button><a class="wordmark" href="/#top">Baivab<span> Sarkar</span></a><nav id="site-nav" aria-label="Main navigation">${[
      ...
      .map(
        ([id, label]) =>
          `<a href="${archive ? "/" : ""}#${id}" data-section="${id}">${label}</a>`,
      )
      .join(
        "",
  ...
  const folder = (p, i) =>
    `<a class="folder folder-${i + 1}" href="/project.html#project-${p.id}"><span class="folder-back" aria-hidden="true"></span><span class="folder-sheets" aria-hidden="true">${[1, 0, 2].map((peek, j) => `<span class="folder-sheet sheet-${j + 1}"><img src="/${peek && i < 3 ? `assets/work/${p.id}-peek-${peek}.webp` : p.image.replace("-1400.webp", "-400.webp")}" width="400" height="${peek && i < 3 ? 260 : Math.round(p.imageHeight / 3.5)}" alt="" loading="lazy" decoding="async"></span>`).join("")}</span><span class="folder-front"><span class="folder-category">${esc(p.category.split(" · ")[0])}</span><span class="folder-title">${esc(p.title)}</span><span class="folder-description">${esc(p.summary)}</span><span class="folder-bottom"><span class="folder-tags">${p.stack
      .slice(0, 2)
 ...
 <section class="writing-section section-space" id="writing" aria-labelledby="writing-title">${heading("NOTES FROM THE PROCESS", "Build. Learn. <em>Write.</em>", "writing-title")}<div class="writing-grid">${articles.map((a, i) => `<article class="writing-card"><a href="${esc(a.url)}"><div class="article-art art-${i + 1}" aria-hidden="true"><span>${esc(["#", "</>", "✓"][i])}</span><small>${["Ideas, in Markdown.", "A better place to write.", "One verified commit."][i]}</small></div><div class="writing-copy"><p class="article-meta"><time datetime="${a.date}">${new Intl.DateTimeFormat("en", { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" }).format(new Date(a.date))}</time> · ${a.readingTime} min read</p><h3>${esc(a.title)}</h3><span class="article-read">Read on DEV ${arrow}</span></div></a></article>`).join("")}</div><div class="section-bottom">${link("https://dev.to/thisisdeveloper", "More notes on DEV")}</div></section>
 <section class="about-section section-space" id="about" aria-labelledby="about-title">${heading("THE HUMAN BEHIND THE CODE", "A little bit about <em>me.</em>", "about-title")}<div class="wallet"><a class="wallet-card wallet-purple" href="https://github.com/ThisIs-Developer/Markdown-Viewer"><small>ON MY WORKBENCH</small><span class="wallet-icon" aria-hidden="true">M↓</span><strong>Markdown Viewer</strong><span>Always another idea.</span></a><div class="wallet-card wallet-yellow"><small>HELLO, I’M</small><img src="/assets/profile/baivab-480.webp" width="480" height="517" alt="Baivab Sarkar" loading="lazy" decoding="async"><strong>Baivab Sarkar</strong><span>Curious by default.</span></div><a class="wallet-card wallet-green" href="#experience"><small>MY FOUNDATION</small><span class="wallet-icon" aria-hidden="true">↗</span><strong>Computer science</strong><span>JIS · Class of 2025</span></a><a class="wallet-card wallet-pink" href="https://dev.to/thisisdeveloper"><small>LEARNING IN PUBLIC</small><span class="wallet-icon" aria-hidden="true">Aa</span><strong>Build & write</strong><span>Notes from the process.</span></a><a class="wallet-card wallet-blue" href="https://github.com/ThisIs-Developer"><small>FIND ME IN THE OPEN</small><span class="wallet-icon" aria-hidden="true">{ }</span><strong>ThisIs-Developer</strong><span>West Bengal, India</span></a></div><form class="quick-ask" hidden><label for="quick-question">Quick ask</label><input id="quick-question" name="question" type="text" maxlength="240" placeholder="Ask about my work, skills, or education" autocomplete="off" required><button type="submit" aria-label="Ask about Baivab">↑</button></form><p id="ask-answer" class="ask-answer" role="status" hidden></p><div class="about-copy"><p>${profile.about}</p><p>${profile.approach}</p></div><details class="experience-details" id="experience"><summary><span>Experience & learning</span><span aria-hidden="true">+</span></summary><div class="timeline">${experience.map((x) => `<article><p class="eyebrow">${esc(x.period)}</p><h3>${esc(x.title)}</h3><p class="timeline-org">${esc(x.organization)}</p><p>${esc(x.description)}</p>${x.link ? link(x.link, x.linkLabel) : ""}</article>`).join("")}<p class="certifications">${certifications.map((x) => `${esc(x.title)} <span>· ${esc(x.date)}</span>`).join("<br>")}</p></div></details><div class="section-bottom">${link(profile.resume, "A little more, in my resume", "button")}</div></section>`;
  const notes = (p) =>
    `<details class="engineering-notes"><summary>Engineering notes <span aria-hidden="true">+</span></summary><div class="notes-body"><p><strong>The problem.</strong> ${esc(p.problem)}</p><p><strong>My contribution.</strong> ${esc(p.contribution)}</p><ul>${p.features.map((f) => `<li>${esc(f)}</li>`).join("")}</ul></div></details>`;
  const archive = `<section class="archive-hero"><a class="text-link" href="/#projects">← Back to selected work</a><p class="eyebrow">PROJECTS & EXPERIMENTS</p><h1>The working <em>notebook.</em></h1><p>Useful tools, team projects, and things made to learn.<br>A closer look at what went into the work.</p></section><section class="archive-section" id="projects" aria-label="Project archive"><div class="archive-filter-space"><div class="archive-filters" hidden><div role="group" aria-label="Filter projects"><button type="button" data-filter="all" aria-pressed="true">All work</button><button type="button" data-filter="featured" aria-pressed="false">Selected</button><button type="button" data-filter="experiments" aria-pressed="false">Experiments</button></div><p role="status" data-filter-status>${all.length} projects</p></div></div><div class="archive-list">${all.map((p, i) => `<article class="archive-item" id="project-${p.id}" data-project-group="${i < 4 ? "featured" : "experiments"}">${p.image ? `<div class="archive-visual tone-${i % 6}">${picture(p)}</div>` : ""}<div class="archive-copy"><p class="eyebrow">${esc(p.category)} · ${esc(p.year)}</p><h2>${esc(p.title)}</h2><p>${esc(p.summary)}</p>${tags(p.stack)}<p class="project-status">${esc(p.status)}</p><div class="work-links">${p.live ? link(p.live, p.liveLabel, "button") : ""}${link(p.source, "Source code")}</div>${notes(p)}</div></article>`).join("")}</div></section>`;
  const shell = (content, isArchive = false) =>
    `<!doctype html>\n<!-- Generated by scripts/build.mjs. Edit data/ or scripts/templates.mjs, then npm run build. -->\n<html lang="en" id="top"><head>${metadata(isArchive)}${isArchive ? "" : '<script src="/game.js?v=20260907" defer></script>'}</head><body>${nav(isArchive)}<main id="main-content" tabindex="-1"><div class="page-surface">${content}</div>${footer(!isArchive)}</main></body></html>`;
  return { "index.html": shell(home), "project.html": shell(archive, true) };
}
